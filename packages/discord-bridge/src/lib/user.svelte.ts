...
// Import Roomy SDK components just for types
import { EntityId, type EntityIdStr } from "@muni-town/leaf";

import { atproto } from "./atproto.svelte";
import { lexicons } from "./lexicons";
import { decodeBase32 } from "./base32.svelte";
...
export const user = {
  ...
  set session(newSession) {
    session = newSession;
    if (newSession) {
      // Store the user's DID on login (only in browser environment)
      if (isBrowser) {
        localStorage.setItem("discord-bridge-did", newSession.did);
      }
  ...
  /**
   * Wait for the catalog ID to be available. This is used by the bridge module
   * to ensure Roomy initialization happens with a valid catalog ID.
   */
  async waitForCatalogId(timeout = 5000): Promise<string | undefined> {
    const start = Date.now();
    while (!catalogId.value && Date.now() - start < timeout) {
  ...
  async init() {
    ...
    if (isBrowser) {
      ...
      // restore the session
      const storedDid = localStorage.getItem("discord-bridge-did");
  ...
  async loginWithHandle(handle: string) {
    if (isBrowser) {
      ...
      throw new Error("Cannot login outside of browser environment");
    }
  },
  ...
  logout() {
    if (isBrowser) {
      localStorage.removeItem("discord-bridge-did");
    }
    session = undefined;
    agent = undefined;
    navigate("/");
